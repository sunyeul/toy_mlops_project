...
def train_model():
    ...
    # This is important since SVM training time increases
    # quadratically with the number of training samples.
    print("Choosing smaller sample to shorten training time...")
    # Set a random seed so that we get the same "random" choices
    # when we try to recreate the experiment.
    np.random.seed(42)
